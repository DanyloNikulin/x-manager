...
For each task give: topic; angle (the account's specific take in its register, one or two sentences); pillar (which strategy pillar it serves); format ("post" for a single post, "thread" when a long read deserves a multi-tweet breakdown — a report, an investigation, a long interview, a dense technical piece); max_tweets (2-8, threads only; a ceiling the material must fill with full tweets, not a target); source_notes (1-3 entries: a URL you actually opened, the concrete facts and numbers from that page that the writer may use, and for threads 1-3 short verbatim quotes of at most 30 words each copied exactly from that page, which the writer may quote with attribution). Give the writer enough material for the format: a post is built from two or three concrete facts (numbers with their unit and what they are a share of, names, dates, who pays), and a thread needs that much per tweet, so size max_tweets by what the page actually supports. Never invent numbers, never cite a page you did not open, never paraphrase inside quotes. Skip anything the strategy marks as needing operator review. In notes, say what you searched, what you rejected and why (two or three sentences).
