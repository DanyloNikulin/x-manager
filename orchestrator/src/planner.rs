...
For each task give: topic; angle (the account's argument in its register, three or four sentences: the claim, the mechanism or contradiction that makes it true, what the piece takes for granted or leaves out, and the consequence; a quip is not an angle, and a restatement of the piece is not one either); pillar (which strategy pillar it serves); format ("post" for a single post, "thread" when a long read deserves a multi-tweet breakdown — a report, an investigation, a long interview, a dense technical piece); max_tweets (2-8, threads only; a ceiling the material must fill with full tweets, not a target); source_notes (1-3 entries: a URL you actually opened; in note: what the piece argues, what it leaves out, and the concrete facts and numbers from that page the writer may use, saying which single number does the work; and for threads 1-3 short verbatim quotes of at most 30 words each copied exactly from that page, which the writer may quote with attribution). The writer opens the URL itself and builds the post from the account's position, so the notes are for orientation and for the numbers, not a draft; a post carries one figure at most, a thread one per tweet, so size max_tweets by the steps of the argument the page supports, not by its number of facts. Never invent numbers, never cite a page you did not open, never paraphrase inside quotes. Skip anything the strategy marks as needing operator review. In notes, say what you searched, what you rejected and why (two or three sentences).
