...
## Approving what waits for a human

A task the worker left as `waiting_approval` shows on the Overview with **Approve** and **Reject** (`POST /api/agent/tasks/:id/review`, `src/lib/task-review.ts`). Approve schedules the worker's draft — a reply keeps its target and goes out on the next tick inside the policy window, a post takes the next open slot, a thread is rebuilt as chained rows — and closes the task as `done`; Reject deletes the draft and closes it as `skipped`. The X API on the account's access tier refuses replies to posts that do not mention the account ("You can only reply to or quote posts where you are mentioned or are the author"), so only replies to inbound mentions can be scheduled: an outbound reply (researcher-suggested) shows **Copy reply** and **Posted by hand** instead of Approve, the operator pastes it into X, and `manual` closes the task with the text in its output. `POST /api/agent/tasks/:id/review` refuses `approve` for such a reply with 409. Scheduling a draft is one SQLite transaction shared with the Drafts page (`POST /api/drafts/:id/schedule`, `src/lib/draft-schedule.ts`): the draft row is claimed with `DELETE … RETURNING`, the scheduled rows are inserted, and the worker task the draft came from is closed, so the two paths can never publish the same draft twice, and a decision that races with another one fails with 409 and writes nothing.
